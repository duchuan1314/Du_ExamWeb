...
<body>
	...
	%>
	<h1 align="center">考题类型管理</h1>
	<span>您是当前第<i>${count }</i>位登陆的用户
	</span>
	<hr>
	<div class="header">
		<a href="<%=request.getContextPath()%>">用户登陆首页</a> <a href=" Listall">用户管理</a>
		<a href="<%=basePath %>ListAllQuestionKuDatil?pageNum=1">题库管理</a> <a
			href="<%=request.getContextPath()%>/QuestionPages/addQuestionPage.jsp">添加考题类型</a>
			...
			<div id="content">
				<table cellspacing="0" cellpadding="0">
					...
					<c:forEach items="${question }" var="question">
						<tr class="two">
							<td>${question.typeCode }</td>
							<td>${question.typeName }</td>
							<td><a href="seeQuestionType?id=${question.id }">查看</a></td>
							<td><a
								href="<%=request.getContextPath() %>/EditQuestionServlet?id=${question.id}&pageNum=${currentPage}">编辑</a></td>
							<td><a
								href="<%=request.getContextPath() %>/deleteQuestionServlet?id=${question.id}&pageNum=${currentPage}">删除</a></td>
						</tr>
					</c:forEach>
				</table>
			</div>
		<div class="wrap"  >
				<div class="fenye">
					<ul>
						...
						<li id="first"><a <c:if  test="${currentPage>1}"> href="<%=basePath%>ListAllQuestion?pageNum=1"</c:if>>首页</a></li>
						<li id="top" onclick="topclick()"><a <c:if  test="${currentPage>1}"> href="<%=basePath%>ListAllQuestion?pageNum=${currentPage-1}"</c:if>>上一页</a></li>
						...
						<li id="down" onclick="downclick()"><a <c:if test="${currentPage<endpageNum}" var="endpageNum">href="<%=basePath%>ListAllQuestion?pageNum=${currentPage+1}"	</c:if>>下一页</a></li>
						<li id="last"><a href="<%=basePath%>ListAllQuestion?pageNum=${endpageNum2}">末页</a></li>
					</ul>
				</div>
			</div>
		</div>
